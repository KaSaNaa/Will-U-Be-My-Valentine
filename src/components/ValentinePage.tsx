...
const ValentinePage: React.FC<ValentinePageProps> = ({
  ...
  const handleOnClick = (response: "yes" | "no") => {
    ...
    if (response === "yes") {
      ...
      const happyAudio = new Audio("/public/assets/happy.mp3");
      happyAudio.play();
      happyAudio.onended = onYesClick || null;
    } else if (response === "no") {
      navigator.vibrate([200, 100, 200]);
      const cryingAudio = new Audio("/public/assets/crying.mp3");
      if (onNoClick) {
        onNoClick();
      }
      cryingAudio.play();
      cryingAudio.onended = () => {
        const cryingAudio2 = new Audio("/public/assets/crying.mp3");
        cryingAudio2.play();
